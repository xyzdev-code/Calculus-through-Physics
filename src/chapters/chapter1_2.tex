...
\marginnote{\footnotesize \textit{However, this does not apply to trigonometric functions. \(\sin^2(x)\) is \(\sin(x) \times \sin(x)\) instead of \(\sin(\sin(x))\). Annoying mathematical notation strikes again. What is even better, is that \(f^{-1}(x)\) denotes the inverse of \(f\).}}
\end{equation*}
\begin{example}
...
If I ask you what is \(\arcsin(B)\), you may say it is \(\frac{b}{c}\). But, what is \(\arcsin(\sin(B))\), well, that is just \(B\), isn't it. Hence, the \(\arcsin\) sort of "cancels" the \(\sin\). We call \(\arcsin\) the \textit{inverse function} or just inverse of \(\sin\). \marginnote{\footnotesize \textit{We will use \(\arcsin\) as the inverse of \(\sin\) instead of \(\sin^{-1}\) as it is clearer.}} Generally, given a function \(f: X \to Y\), \(f^{-1}\) denotes the inverse function that satisfies:
\begin{equation}
  f \circ f^{-1}(y) = y, y \in Y \text{\, and \,} f^{-1} \circ f(x) = x, x \in X
\end{equation}

One common misconception is that \(f(f^{-1}(x))=f^{-1}(f(x))\). While this is true for many functions like \(f(x)=x+1\), it is \textit{not} always true. For example, take the following function:
\begin{equation*}
  f(x)=\sin(x), f: \mathbb{R} \to \{y: -1 \leq y \leq 1 \} 
\end{equation*}
...
\begin{Exercise}
  \enumerate{
    \item{Given \(f(x)=\frac{x^2+3}{2}\) and \(g(x)=\tan(x)+1\), evaluate:
        \begin{enumerate}
          \item{\(f(g(\frac{\pi}{4}))\)}
          \item{\(f^{3}(1)\)}
          \item{\(f(g(\arcsin(0)))\)}
    ...
    \item{Given that \(h(x)=(x-7)^2, \, x \geq 7\), find the inverse of \(h\).}
    \item{Hence, evaluate:
        \begin{enumerate}
          \item{\(h^{-1}(1)\)}
