...
What is more interesting, perhaps, is taking the \textit{output} of a function and use it as the \textit{input} to another function. You may also think of it as "piping" one function into another. This is called \emph{function composition}.
...
\begin{figure}[h]
  ...
  \end{tikzpicture}
  \caption{Visual representation of function composition.\protect\footnotemark}
\end{figure} 
 \footnotetext{Note that the rectangles are functions and circles are values.}
...
When you compose the same function \(n\)-times, the notation for it is\footnote{However, this does not apply to trigonometric functions. \(\sin^2(x)\) is \(\sin(x) \times \sin(x)\) instead of \(\sin(\sin(x))\). Annoying mathematical notation strikes again. What is even better, is that \(f^{-1}(x)\) denotes the inverse of \(f\).}:
\begin{equation*}
...
\begin{problem}[1]
  The \emph{dirac delta function} is defined as 
  \begin{equation}
    \delta(x)=\begin{cases}
      \infty & x=0 \\
      0 & \textit{otherwise}
