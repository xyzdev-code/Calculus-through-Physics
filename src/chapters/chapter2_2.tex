...
\begin{Exercise}
  ...
  \begin{enumerate}
    ...
    \end{enumerate} }
  \item{Prove or disprove that the dot product \textit{commutes}. }
\marginnote{\footnotesize \textit{An operation \(\circ\) is commutative if \(x\circ y = y \circ x\).}}
...
\begin{Solution}
  ...
  \marginnote{\footnotesize \textit{Note that \(\vec{a}\cdot\vec{a}=(||\vec{a}||)^2\) as evident from the second definition of \Cref{dotproduct}.}}
    \begin{align*}
