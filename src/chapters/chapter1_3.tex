...
\begin{definition}
  An \textbf{injective} function, \(f\), or a \textit{one-to-one} function, is a function such that every distinct output of the function corresponds to only \textit{one} distinct input, that is \footnotemark:  
  \begin{equation*}
    f(x_1) = f(x_2) \text{\, implies \,} x_1 = x_2 
  \end{equation*}
\end{definition}
\footnotetext{By contraposition, \(x_1 \neq x_2\) implies \(f(x_1) \neq f(x_2)\). The contraposition of a statement "p then q" is "not q then p".}
...
Here, the function is \textit{not} injective. As you can see in \Cref{non_inj}, the output \(y\) is associated with more than one input in \(X\). With this in mind, it is time to talk about \emph{surjection}. 
\begin{definition}
...
From \Cref{nsurj}, we can see that \(y\) has no input associated with it. As a result, the function here is not surjective. Lastly, \emph{bijective} functions can't be any easier.
\begin{definition}
  A \textbf{bijective} function is a function that is both injective and surjective. 
\end{definition}


However, bijective functions have a superpower. A function is only bijective \textit{if and only if} it has an inverse.\footnote{P iff Q, or "P if and only if Q", means that "if P then Q" and "if Q then P".}
\begin{theorem}
