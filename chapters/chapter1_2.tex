...
\marginnote{\footnotesize \textit{However, this does not \\ apply to trigonometric functions. \(\sin^2(x)\) is \(\sin(x) \times \sin(x)\) instead of \(\sin(\sin(x))\).}}
\end{equation*}
\begin{example}
...
If I ask you what is \(\arcsin(B)\), you may say it is \(\frac{b}{c}\). But, what is \(\arcsin(\sin(B))\), well, that is just \(B\), isn't it. Hence, the \(\arcsin\) sort of "cancels" the \(\sin\). We call \(\arcsin\) the \textit{inverse function} or just inverse of \(\sin\). Generally, given a function \(f\), \(f^{-1}\) denotes the inverse function such that \(f(f^{-1}(x))=x\). Notice that if we apply both of the functions together, we get the identity function, that is:
\begin{equation}
  f \circ f^{-1} = id_x \text{\,where \,} id_x \text{\,is the identity function}
  \marginnote{\footnotesize \textit{The identity function is just \(f(x)=x\)}}
\end{equation}
...
\begin{example}
  ...
  \begin{align*}
    ...
    x&=3y-7
  \end{align*}
  Hence, the inverse function is \(f^{-1}(x)=3y-7\).
\end{example}
