...
As you can see in \Cref{inj}, every output in \(Y\) of the function is associated with only one input in \(X\), hence the function is \textit{injective}.
\begin{figure}[h]
\centering
\begin{tikzpicture}
  \filldraw [red!60] (0,0) ellipse (1cm and 2cm) node[below, yshift=-60, color=black]{X}; 
...
From \Cref{inj} and \Cref{non_inj}, both functions are surjective as every output is associated with at least one input. Do take note that it does not matter how many inputs associate to one output, as long every output is associated with at least one input, the function is surjective.
\newpage
\begin{figure}[h]
\centering
\begin{tikzpicture}
...
\end{theorem}
\begin{proof}
  To begin such proofs where there is a "if and only if" clause, we will prove both cases separately. First, we claim that if the function has an inverse, then it is bijective. To prove a bijection, we must prove that it is injective and surjective. Suppose \(a_1 , a_2 \in A\)
\end{proof}
